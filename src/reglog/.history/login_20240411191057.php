<?php
// XAMPP database connection parameters
$servername = "localhost"; // Change this to your database server name (usually localhost)
$username = "root"; // Change this to your database username (usually root)
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["username"]) && isset($_POST["password"]) && !empty($_POST["username"]) && !empty($_POST["password"])) {
        ...
        if ($username === "admin" && $password === "password") {
            // Successful login, set the session variable for the username
        ...
        $notification = "Please enter both username and password";
    }
}

// Set the notification message if it's set in the session
if (isset($_SESSION["notification"])) {
    $notification = $_SESSION["notification"];
    unset($_SESSION["notification"]); // Clear the notification from the session
}
...
if (isset($_SESSION["username"])) {
    ...
    if ($result->num_rows > 0) {
        // User found, display user-specific data
        $row = $result->fetch_assoc();
        $userId = $row["id"];
        $userEmail = $row["email"];
    } else {
        // User not found, display an error message
        $notification = "User data not found";
    }
}
...
<body>
  ...
  <div class="login-container">
    ...
    <form id="login-form">
      ...
      <input type="submit" value="Login" onclick="showNotification()">
    </form>
    <div class="password-strength" id="password-strength">Password Strength: <span id="password-strength-indicator"></span></div>
    <div class="new-member">New here? <a href="#" class="sign-up">Sign up now</a></div>
  </div>
