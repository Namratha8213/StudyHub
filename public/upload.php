            ...
            if ($fileError === 0) {
                ...
                if (move_uploaded_file($fileTmpName, $targetPath)) {
                    ...
                    if ($insertStmt->execute()) {
                        ...
                        echo json_encode(array("success" => false, "error" => "Error uploading file: " . $insertStmt->error));
                    }
                    $insertStmt->close();
                } else {
                    echo json_encode(array("success" => false, "error" => "Error moving file to destination."));
                }
            } else {
                echo json_encode(array("success" => false, "error" => "Error uploading file."));
            }
        } else {
            echo json_encode(array("success" => false, "error" => "Subject not found for the selected semester."));
        }
        $stmt->close();
    } else {
        echo json_encode(array("success" => false, "error" => "Please select semester, subject, and upload a file."));
    }
}
$conn->close();
?>